...
def createFileTable():
	try:
		cursor.execute("CREATE TABLE [Filebchn](username VARCHAR(50), test VARCHAR(50), dt VARCHAR(50), uploader VARCHAR(50), filename VARCHAR(50) UNIQUE)")
		cursor.commit()
	except:
		pass
...
def addFile(username, test, dt, uploader, filename):
	try:
		command = 'INSERT INTO [Filebchn] VALUES (?,?,?,?,?)'	
...
def getUserFromFile(filename):
	try:
		command ='SELECT username FROM [Filebchn] WHERE filename=?'
...
def getTestFromFile(filename):
	try:
		command ='SELECT test FROM [Filebchn] WHERE filename=?'
		cursor.execute(command,filename)
		retValue=cursor.fetchone()[0]
		cursor.commit()
		return retValue
...
def getDateFromFile(filename):
	try:
		command ='SELECT dt FROM [Filebchn] WHERE filename=?'
		cursor.execute(command,filename)
		retValue=cursor.fetchone()[0]
		cursor.commit()
		return retValue
...
def getUploaderFromFile(filename):
	try:
		command ='SELECT uploader FROM [Filebchn] WHERE filename=?'
		cursor.execute(command,filename)
		retValue=cursor.fetchone()[0]
		cursor.commit()
		return retValue
...
def getFileListFromUser(user):
	try:
		...
		op=op+'<th>Link</th>\n'
		op=op+'</tr>\n'
		command= 'SELECT test, dt, uploader, filename FROM [Filebchn] where username=?'
